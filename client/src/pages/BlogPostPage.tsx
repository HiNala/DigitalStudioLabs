...
const processPostContent = (content: string, postTitle: string) => {
  ...
  // First, let's handle the duplicate Introduction problem specifically - this is a common issue
  let processedContent = content;
  
  // If we have multiple h1/h2/h3 elements with "Introduction", keep only one h2 with "Introduction"
  // First, check if there are multiple "Introduction" headings
  const introHeadingMatches = processedContent.match(/<h[1-3][^>]*>Introduction[\.!\s]*<\/h[1-3]>/gi);
  if (introHeadingMatches && introHeadingMatches.length > 1) {
    // Remove all Introduction headings initially
    processedContent = processedContent.replace(/<h[1-3][^>]*>Introduction[\.!\s]*<\/h[1-3]>/gi, '');
    // Add one single Introduction heading at the appropriate position
    const firstHeadingMatch = processedContent.match(/<h[1-3][^>]*>/i);
    if (firstHeadingMatch && firstHeadingMatch.index) {
      ...
      processedContent = 
        ...
        '<h2 class="mb-2" id="introduction">Introduction</h2>' + 
        processedContent.slice(insertPosition);
    } else {
      // If there's no heading found, just add it at the beginning of the content
      processedContent = '<h2 class="mb-2" id="introduction">Introduction</h2>' + processedContent;
    }
  }
  ...
  return processedContent
    ...
    .replace(new RegExp(`<h1[^>]*>${titleRegexBase}[\.!\s]*<\/h1>`, 'gi'), '')
    .replace(new RegExp(`<h2[^>]*>${titleRegexBase}[\.!\s]*<\/h2>`, 'gi'), '')
    .replace(new RegExp(`<h3[^>]*>${titleRegexBase}[\.!\s]*<\/h3>`, 'gi'), '')
    
    // Remove extra line breaks that create large gaps
    .replace(/(<\/h[1-6]>)\s*<br\s*\/?>\s*<br\s*\/?>\s*(<p)/gi, '$1$2')
    .replace(/(<\/h[1-6]>)\s*<br\s*\/?>\s*(<p)/gi, '$1$2')
    .replace(/(<\/p>)\s*<br\s*\/?>\s*<br\s*\/?>\s*(<h)/gi, '$1$2')
    .replace(/(<\/div>)\s*<br\s*\/?>\s*<br\s*\/?>\s*(<h)/gi, '$1$2')
    
    // Remove unnecessary div containers that create spacing
    ...
    .replace(/<h1/g, '<h1 class="text-3xl font-bold mb-2 mt-8"')
    .replace(/<h2/g, '<h2 class="text-2xl font-bold mb-2 mt-6"')
    ...
    .replace(/<p class="mb-3 leading-7">\s*<\/p>/g, '') // Remove empty paragraphs
            ...
            {isLoading ? (
              ...
              >
                ...
                <div className="bg-transparent backdrop-blur-sm overflow-hidden">
                  ...
                  <div 
                    className="prose dark:prose-invert prose-lg max-w-none prose-headings:font-poppins prose-a:text-[#00A0B0] hover:prose-a:text-[#4D4DFF] prose-img:rounded-xl prose-p:text-[#8B949E]/90 prose-p:leading-relaxed prose-h1:text-3xl prose-h2:text-2xl prose-h3:text-xl prose-headings:font-bold prose-headings:mb-0 prose-headings:mt-0 prose-headings:leading-snug prose-blockquote:border-l-[#4D4DFF] prose-blockquote:bg-[#161B22]/60 prose-blockquote:p-4 prose-blockquote:rounded-r-md [&>h2+p]:mt-0 [&>h3+p]:mt-0"
